  ...
  return (
    <section className="w-full relative overflow-hidden py-16 sm:py-20">
      <div className="mx-auto px-4 sm:px-6 lg:px-8 w-full max-w-7xl">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-12" ref={statsRef}>
          {stats.map((stat, index) => (
            <div key={index} className="text-center group scroll-animate">
              ...
              <div className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-2" style={{ color: '#000080' }}>
                {stat.number}
              </div>
              <div className="text-sm sm:text-base font-medium" style={{ color: 'var(--text-on-light-muted)' }}>
                {stat.label}
              </div>
            </div>
          ))}
